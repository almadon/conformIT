...
- **`docs/`** — the standards themselves. Prose, with reasoning and cost
  stated, because a rule whose justification isn't recorded gets re-litigated
  or quietly reversed by someone who assumed it was arbitrary.
...
These standards were derived from patterns already present and independently
repeated across nine of the author's projects, not invented from scratch. The
specific sources are cited inline in each document, so a reader can judge
whether a rule earned its place or is one project's habit dressed up as a
principle.
...
- **Everything else** — templates, hooks, scripts — [MIT](LICENSE). They are
  meant to be copied into your repo without dragging an attribution
  obligation into your codebase.
